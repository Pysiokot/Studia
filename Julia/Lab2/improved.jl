...
# Generates random address object (with random number).
...
function bfs(;visited=Set{GraphVertex}(), remaining=Set(graph))
...
function check_euler()
  if length(partition()) == 1
    ...
    return (all(map(v -> iseven(length(v.neighbors)), graph)) ? "true" : "false")
  end
    "Graph is not connected"
end

#added multiple dispatch
function make_node_str(n::Person)
  "Person: $(n.name)\n"
end

function make_node_str(n::Address)
...
function graph_to_str()
  ...
  for v in graph
    ...
    n = v.value
    #added function make_node_str
    node_str = make_node_str(n)
